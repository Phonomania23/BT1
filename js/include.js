  ...
  await Promise.all(
    slots.map(async (placeholder) => {
      ...
      try {
        ...
        // Добавляем fallback для отладки
        placeholder.innerHTML = `<div style="padding: 10px; background: var(--surface-2); border: 1px solid var(--error); color: var(--error); border-radius: 4px;">
          Ошибка загрузки: ${absUrl}
        </div>`;
      }
    })
  );
